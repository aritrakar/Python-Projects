...
WAKE = "jarvis"
GREETINGS = ["Hello", "I'm ready", "At your service",
             "The name's Jarvis, just Jarvis", "What's up"]
BYE = ["Bye!", "May the Force be with you!",
       "Live long and prosper!", "See you later!"]

# PROMPTS: Put in JSON file
QUIT_PROMPTS = ["quit", "see you later", "exit"]
CALENDAR_PROMPTS = ["what do i have", "do i have plans", "am i busy"]
NOTE_PROMPTS = ["take a note", "make a note",
                "write this down", "remember this"]
DOCUMENT_PROMPTS = ["create a document",
                    "create a new document", "open google docs", "open docs"]
...
'''
def speak(text): # Using gTTS. Does NOT support continuous interaction. 
...
def getEvents(day, service):  # Calendar: Works. Can only get Events
...
def main():
    while True:
        ...
        # Calendar
        for phrase in CALENDAR_PROMPTS:
            if phrase in text:
                date = getDate(text)
                if date:
        ...
        # Weather
        for phrase in WEATHER_PROMPTS:
            if phrase in text:
                txt = text.replace(phrase, "")
                getWeather(txt)
        ...
        # Spotify support
        if text.count("play") > 0:
            getSong(text, SPOTIFY_OBJECT)

        if text.count("pause") > 0:
            togglePlayback("pause", SPOTIFY_OBJECT)

        if text.count("resume") > 0:
            togglePlayback("resume", SPOTIFY_OBJECT)
